...
def version_generalizada(m, k):
    ...
    # Obtener los nombres de los animales
    animal_keys = [animal[0] for animal in animals]
    ...
    for _ in range((m-1)*k):
        ...
        scene_animals = random.sample(animal_keys, 3)
        
        # Agregar la escena a la apertura
        aperture.append(scene_animals)
...
if __name__ == "__main__":
    ...
    functions_to_test = [
        ConcertZoo2.sort_aperture,
        ConcertZoo2.sort_rest_of_show,
        ConcertZoo2.most_appearances,
        ConcertZoo2.least_appearances,
        ConcertZoo2.max_grandeur_scene,
        ConcertZoo2.min_grandeur_scene,
        ConcertZoo2.average_grandeur,
    ]

    # for function in functions_to_test:
    header = f"Tiempos de la solución para {ConcertZoo2.average_grandeur.__name__}:"
    times = measure_execution_time(versions, ConcertZoo2.average_grandeur)
    table = tabulate(enumerate(times, start=1), headers=["Ejecución", "Tiempo (s)"], tablefmt="pretty")
    print(f"\n{header}\n{table}")
    
    # Gráfica de tiempos de ejecución
    plot_execution_times(versions, ConcertZoo2.average_grandeur, times)
